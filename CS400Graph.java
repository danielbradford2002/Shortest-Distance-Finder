...
//            Path extended = new Path(copyPath.start);
//            extended.distance = copyPath.distance;
//            try {
...
//                System.out.println(this.dataSequence.get(0));
                for(NodeType ver: copyPath.dataSequence){
//                    if(ver != null && this.dataSequence != null) {
                        this.dataSequence.add(ver);
//                    }
                }
...
//            Iterator extendedIt = extended.dataSequence.iterator();
//            for(Iterator it = copyPath.dataSequence.iterator(); it.hasNext();){
//                extendedIt.next() = it.next();
//            }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        //    //go over loop until frontier is empty and then dequeue from frontier and then that will
//    // return a path and then look at the end of the path which will be a vertex and then look at
//    // the vertex's linked list of edges that leave the vertex. then i can construct new paths
//    // with the Path extend constructor. then i need to check the new paths target to see if the
//    // target of that path has been visited or not. then compare the distance to update the
//    // shorter path.
        if(start==null || end == null){
            throw new NoSuchElementException("error. start or end is null");
        }
        ...
        frontier.add(p);
            while (!frontier.isEmpty()) {
                ...
                if (curr.end.data.equals(end)) {
                    System.out.println(curr.end.data + "end");
                    System.out.println("reached end and shortest dist: " + curr.distance);
                    return curr;
                }
                for (Edge edge : curr.end.edgesLeaving) {
                    if (!visited.contains(edge.target)) {
                        Path pa = new Path(curr, edge);
                        frontier.add(pa);
                        System.out.println("adding dist: " + pa.distance);
                    }
                }
            }
